...
- 말투는 간결하게 작성한다.
- 답변 문서는 두 섹션으로 구성한다.
  1. **쉬운 설명**: 문서 맨 앞에 5살 아이에게 설명하듯 쉬운 비유로 핵심만 짧게 요약하는 섹션을 하나 둔다. 이 섹션은 권장 사항이며 분량을 최대한 채우라는 뜻이 아니다 — 비유로 핵심이 전달되면 한두 문장으로 끝내도 된다.
  2. **일반 설명**: 그 다음 이어지는 본문에서 관련 전문 용어를 포함해 기술적으로 자세히 설명한다.
- 모든 답변은 `.md` 파일로 생성한다.
